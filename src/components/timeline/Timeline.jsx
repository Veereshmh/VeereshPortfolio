...
function Timeline() {
  ...
  return (
    <div className="w-[80%] max-auto rounded-lg mx-auto bg-gray-50 py-10 mb-20">
      <div className="relative px-4">
        {/* Vertical line */}
        {/* On small screens: line on left, full height */}
        {/* On md and above: line on right */}
        <div className="absolute top-0 h-full bg-gray-300 w-0.5 left-8 md:left-auto md:right-0"></div>

        {items.map((item, index) => {
          const isLeft = true; // always left side on md and above

          return (
            <div
              key={index}
              // Full width on small screens, flex row reversed on md for left alignment
